...
describe('bot.ts Speaking Listeners - Null Safety', () => {
  ...
  describe('when guild.members.fetch returns null/undefined', () => {
    it('should handle null member from fetch gracefully', async () => {
      // WHY: Discord API can return null for members that have left or are inaccessible.

      const mockGuild: Partial<Guild> = {
        id: 'test-guild',
        members: {
          fetch: vi.fn().mockResolvedValue(null), // Returns null
        } as any,
      };

      vi.mocked(mockClient.guilds.fetch).mockResolvedValue(mockGuild as Guild);

      // Simulate the bot.ts speaking handler
      const handler = async (userId: string, guildId: string) => {
        try {
          const guild = await mockClient.guilds.fetch(guildId);
          const member = await guild.members.fetch(userId);

          // Null check before accessing member properties
          if (!member) {
            mockLogger.warn({ userId, guildId }, 'Member not found');
            return;
          }
          ...
          if (member.voice.channel) {
            ...
            if (nonBotCount >= 2) {
              await mockAfkDetection.resetTimer(guildId, userId);
            }
          }
        } catch (error) {
          mockLogger.error({ error, userId, guildId }, 'Failed to check threshold for reset');
        }
      };

      await expect(handler('user-null', 'test-guild')).resolves.not.toThrow();

      expect(mockLogger.warn).toHaveBeenCalledWith(
        { userId: 'user-null', guildId: 'test-guild' },
        'Member not found'
      );
      expect(mockAfkDetection.resetTimer).not.toHaveBeenCalled();
    });

    it('should handle undefined member from fetch gracefully', async () => {
      // WHY: Undefined return values should be handled the same as null.

      const mockGuild: Partial<Guild> = {
        id: 'test-guild',
        members: {
          fetch: vi.fn().mockResolvedValue(undefined), // Returns undefined
        } as any,
      };
      ...
      const handler = async (userId: string, guildId: string) => {
        try {
          ...
          if (!member) {
            mockLogger.debug({ userId, guildId }, 'Member is null or undefined');
            return;
          }
      ...
      await expect(handler('user-undef', 'test-guild')).resolves.not.toThrow();

      expect(mockLogger.debug).toHaveBeenCalledWith(
        { userId: 'user-undef', guildId: 'test-guild' },
        'Member is null or undefined'
      );
    });
  });

  describe('when member.voice.channel is null', () => {
    it('should handle null voice.channel gracefully', async () => {
      // WHY: User might have disconnected between speaking event and fetch.
      ...
      const mockMember: Partial<GuildMember> = {
        ...
        voice: {
          channel: null, // User not in voice
        } as VoiceState,
      };
      ...
      const handler = async (userId: string, guildId: string) => {
        mockLogger.debug({ userId, guildId }, 'User started speaking, resetting AFK timer');
        ...
        try {
          ...
          if (member.voice.channel) {
            ...
            mockLogger.debug({ userId, guildId }, 'User not in voice channel');
          }
        } catch (error) {
          mockLogger.error({ error, userId, guildId }, 'Failed to check threshold for reset');
        }
      ...
      await expect(handler('user-disconnected', 'test-guild')).resolves.not.toThrow();

      // Should not attempt to reset timer
      expect(mockAfkDetection.resetTimer).not.toHaveBeenCalled();
      expect(mockLogger.debug).toHaveBeenCalledWith(
        { userId: 'user-disconnected', guildId: 'test-guild' },
        'User not in voice channel'
      );
    });

    it('should handle undefined voice.channel gracefully', async () => {
      // WHY: voice.channel could be undefined instead of null.
      ...
      const mockMember: Partial<GuildMember> = {
        ...
        voice: {
          channel: undefined, // Undefined instead of null
        } as VoiceState,
      };
      ...
      await expect(handler('user-no-voice', 'test-guild')).resolves.not.toThrow();

      expect(mockAfkDetection.resetTimer).not.toHaveBeenCalled();
    });
  });

  describe('when fetch operations throw errors', () => {
    it('should catch and log guild.fetch errors', async () => {
      // WHY: Network errors, invalid guild IDs, or permissions issues can cause fetch to reject.
      ...
      await expect(handler('user-123', 'invalid-guild')).resolves.not.toThrow();
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        ...
        'Failed to check threshold for reset'
      );
      expect(mockAfkDetection.resetTimer).not.toHaveBeenCalled();
    });

    it('should catch and log member.fetch errors', async () => {
      // WHY: Member fetch can fail if the user left the guild or permissions changed.
      ...
      const handler = async (userId: string, guildId: string) => {
        mockLogger.debug({ userId, guildId }, 'User stopped speaking, starting AFK tracking');
        ...
        try {
          ...
          if (member.voice.channel) {
            const nonBotCount = member.voice.channel.members.filter(m => !m.user.bot).size;
            if (nonBotCount >= 2 && !mockAfkDetection.isTracking(guildId, userId)) {
              await mockAfkDetection.startTracking(guildId, userId, member.voice.channel.id);
            }
          }
        } catch (error) {
          mockLogger.error({ error, userId, guildId }, 'Failed to start tracking after user stopped speaking');
        }
      };

      await expect(handler('missing-user', 'test-guild')).resolves.not.toThrow();
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        ...
        'Failed to start tracking after user stopped speaking'
      );
      expect(mockAfkDetection.startTracking).not.toHaveBeenCalled();
    });

    it('should handle rate limit errors without crashing', async () => {
      // WHY: Discord rate limits are a reality - the bot must handle them gracefully.

      const rateLimitError = new Error('Rate limited');
      vi.mocked(mockClient.guilds.fetch).mockRejectedValue(rateLimitError);
      ...
      await expect(handler('user-ratelimit', 'test-guild')).resolves.not.toThrow();
  ...
  describe('when channel.members operations fail', () => {
    it('should handle when channel.members.filter throws', async () => {
      // WHY: Corrupted data structures or Discord.js bugs could cause collection methods to throw.

      const corruptedChannel = {
        id: 'corrupt-channel',
        members: {
          filter: vi.fn(() => {
      ...
      const mockMember: Partial<GuildMember> = {
        ...
        voice: {
          channel: corruptedChannel as any,
        } as VoiceState,
      };
      ...
      await expect(handler('user-corrupt', 'test-guild')).resolves.not.toThrow();

      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.objectContaining({
          error: expect.objectContaining({
            message: 'Collection corrupted',
          }),
        }),
        'Failed to check threshold for reset'
      );
    ...
    it('should handle when channel.members is null', async () => {
      // WHY: Defensive programming - handle impossible states gracefully.

      const nullMembersChannel = {
        id: 'null-members-channel',
        members: null,
      };

      const mockMember: Partial<GuildMember> = {
        id: 'user-null-members',
        user: {
          id: 'user-null-members',
          bot: false,
        } as any,
        voice: {
          channel: nullMembersChannel as any,
        } as VoiceState,
      };
      ...
      const handler = async (userId: string, guildId: string) => {
        try {
          ...
          if (member.voice.channel) {
            // Check if members exists before filtering
            if (!member.voice.channel.members) {
              mockLogger.warn({ userId, guildId }, 'Channel members collection is null');
              return;
            }
      ...
      await expect(handler('user-null-members', 'test-guild')).resolves.not.toThrow();

      expect(mockLogger.warn).toHaveBeenCalledWith(
        { userId: 'user-null-members', guildId: 'test-guild' },
        'Channel members collection is null'
      );
    });
  });
});
